...
- **투표/의사결정** — 차량 편성, 메뉴, 참석 확인 등 후보를 올리고 실시간 투표 + 댓글
- **정산** — 지출을 1/N 또는 항목별로 분배 → 누가 누구에게 얼마 보낼지 **최소 송금**으로 자동 정리
- **일정 · 공지 · 준비물** — 타임테이블, 공지, 공용/개인 준비물 체크
- **로그인 없음** — 명단에서 본인 이름만 선택하면 입장
...
├── config.js      ← ★ 명단·일정·공지·준비물·Firebase 설정 (여기만 고치면 됨)
...
- 이 사이트는 **로그인이 없다.** 링크를 아는 사람은 누구나 이름을 골라 들어와 투표·정산을 수정할 수 있다 (크루용으로 적절, 외부 공개용은 아님).
