...
> 마지막 갱신: 2026-06-21 (주말 집 작업). 집에서는 `git pull` 후 이 파일부터 읽기.

## 지금 상태 (한 줄)
다중 동호회 스포츠 커뮤니티 앱. **라이브 정상 배포 + 모든 변경 푸시·머지 완료.** 이번 주말 RSVP①~⑤·일정 투표 복구·클라이밍 색표·썸네일 작업 완료(아래 2026-06-21 섹션). 사용성 평가(§7) 일부 잔여.
...
## 이번 주말(2026-06-21) 완료 — 전부 라이브 (PR #4·#5 머지)
> ⚠️ **멀티세션 주의**: 이 주말 다른 세션이 `main`에 RSVP②③를 **직접 푸시** → PR 머지 때 충돌남(해소 완료). **작업 시작 전 `git fetch origin main`으로 최신 확인**하고, 같은 영역(`app.js` RSVP/일정) 동시 작업은 피할 것.

### 일정 RSVP (활동별 참석) ①~⑤
- **①** 타임테이블 항목마다 참석/미정/불참 칩+집계 (`schedule/{id}/rsvp/{memberId}`, 분모 `sessionMemberIds()`).
- **②** 카풀 탭 '내 이동수단' 토글(운전↔탑승, `set-mycar` → 전역 `members/{me}/hasCar`).
- **③** 일정 탭 운영진 '미응답 N명 알림'(`nudge-rsvp` — claimed·본인 제외 일괄 notify).
- **④** 예약 정원 대조 — 일정에 `cap` 설정 시 참석 인원과 대조 경고("16명분 예약·참석 12 — 4자리 남음"/초과 빨강).
- **⑤** 변경 마감일+취소정책 — 일정에 `lockDate`/`cancelNote`. 마감 후 RSVP 변경 시 정책 confirm(소프트, 하드락 ✕).
- **버그픽스**: 일정 수정이 `Store.set`으로 노드 교체 → `rsvp` 소실하던 것 `Store.update`로 보존.

### 일정 투표 복구
- v37때 화면 연결 끊겼던 세션 투표(숙소/코스 결정) 복구. `viewAlert`에 `vote` 분기 복원 + 일정 홈 '결정' 섹션. 데이터/함수(`viewVote`/`viewPollDetail`/`pollMiniCard`)는 온전했음.

### 클라이밍 완등 — 암장 색깔→V 자동 환산
- `config.js` `climbGyms`: 전용 색표 5곳(더클라임·피커스·서울숲·손상원·클라이밍파크) + 기타(직접 V). `climbGradeBase`=공용 기본 스케일.
- `formClimb`: 암장 선택→색 칩→"빨강≈V3" 자동 환산(`pick-climb-color`). `saveClimb`은 환산 V + **원본(`gymId`·`color`) 함께 저장** → V 언제든 재보정 가능.
- 같은 색도 암장별 다른 V(피커스 빨강 V0 vs 더클라임 V3) → 순위는 V로 정규화. ⚠️ 색→V는 근사치. 손상원·클파는 순서 이례적(현장확인), 서울볼더스·더플라스틱은 색표 미확보로 제외(기타로 입력).
- 데이터 출처: spiri7·dcinside는 봇 **403** 차단이라 자동수집 불가 → 리뷰 블로그+구글시트 수동 큐레이션.

### 링크 공유 카드(썸네일)
- `og-cover.png` Pretendard로 재생성('내게 딱 맞는 동호회' 제거, CREWFIT+모토만). 메타 제목/설명 중복 제거(제목 "크루핏 (CREWFIT)", 설명 기능 위주). 재생성 스크립트는 `@resvg/resvg-js`+`pretendard` npm(레포 미커밋 — 필요시 복원).
...
6. **클라이밍 색표 보정**: 서울볼더스·더플라스틱 색 순서 확보 시 `config.js climbGyms`에 추가. 손상원·클라이밍파크는 순서 이례적이라 현장 체감과 다르면 V숫자만 보정(원본 `color` 저장돼 안전).
7. **§7 RANK-1/6 잔여**: 클라이밍 폼 색표화로 'V0 기본 실수'는 상당 부분 해소(기타만 V0 기본). 멤버 미선택 무음(`if(!member)return`)은 아직 — 인라인 경고 추가 여지.
